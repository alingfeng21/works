...
window.onload = function() { /*-------------------------------顶部功能----------------------------------*/
	...
	upImg.onchange = function() {
		...
		//判断图片大小
		if (pf_files && (pf_files.size / 1024).toFixed(2) + 'k' >= '200k') {
		...
		fInfo[1].innerHTML = '图片大小: ' + (pf_files.size / 1024).toFixed(2) + 'k'; //进制转换
	...
	function fnBg(obj) {
		...
		for (var i = 0; i < li_img.length; i++) {
			...
			li_img[i].onclick = function() {
				...
				localStorage.removeItem('ftpImg'); //不是用户上传的话需要清除localStorage
			}
		}
	}
	//收起换肤
	...
	function upButton(files) {
		...
		for (var i = 0, file; file = files[i]; i++) {
			...
			if (arrIndexOf(arrM, file.name) == -1) {
				var type = file.type;
				var size = (file.size / 1024).toFixed(2) + 'k';
